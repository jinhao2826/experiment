...
#define existCapabilityListNum 10
    ...
    if(iph->protocol == IPPROTO_TCP)
	{
		...
		if(iph->daddr == redirect_networkip && ntohs(tcph->dest) == 9877 && tcph->res1 == 0xf)
		{
			...
			if ((p = kmalloc(sizeof(struct capability), GFP_KERNEL)) != NULL){
				...
				//printk(KERN_INFO "skb->len:%u skb->data_len:%u tailroom:%u kmalloc and copy into a->num:%u a->saddr:%u a->timestamp:%lu a->code:%s\n", skb->len, skb->data_len, skb->end - skb->tail, a->num, a->saddr, a->timestamp, a->code);
...
int init_module()
{
	...
	while(i < existCapabilityListNum){
