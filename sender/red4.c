	...
	if(iph->protocol == IPPROTO_TCP)
	{
		...
		if(iph->daddr == middlebox_networkip && ntohs(tcph->dest) == 9877 && !tcph->fin)
		{
			...
			if(jiffies <= (temp->TA + sendperiod) && temp->PID <= capability_sum){
				...
				//memset(cap->code,'\0', capability_room - 12);
		...
		if(iph->saddr == redirect_networkip && ntohs(tcph->source) == 9877 && tcph->ack && (tcph->res1 != 0))
		{
			...
			if((temp2 != NULL) && jiffies <= (temp2->TA + interval) && temp2->i <= capability_sum){
				...
				while(res1 >= 1){
					...
					printk(KERN_INFO "get capability num=%u saddr=%u timestamp=%lu code=%s <res1:%u>\n", cap2->num, cap2->saddr, cap2->timestamp, cap2->code, res1);
					spin_lock(&mylock);
					checkgetcapability(ctab, cap2->num, cap2->saddr, cap2->timestamp, cap2->code);								  spin_unlock(&mylock);
